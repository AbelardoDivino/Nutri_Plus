...
function App() {
  ...
  return (
    <div className="App">
      ...
      {tela === 'login' && <Login onLogin={(tipo) => setTela(tipo)} onCadastrar={() => setTela('cadastrar')} onRecuperarsenha={() => setTela('esqueceuasenha')} />}
      {tela === 'cadastrar' && <Cadastrar onVoltar={() => setTela('login')} onCadastrado={() => setTela('usuario')} />}
      {tela === 'usuario' && <Campousu />}
      {tela === 'admin' && <Admin />}
      {tela === 'esqueceuasenha' && <Esqueceuasenha onVoltar={() => setTela('login')} />}
